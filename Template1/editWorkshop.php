<?php
    ...
    $result = mysqli_query($conn, "select * from tbl_workshop where Workshop_ID= ".$Workshop_ID) or die(mysqli_error($conn));
    ...
    if (isset($_POST["btnsubmit"])){
        $title=$_POST["title"];
        $desc=$_POST["desc"];
        $dateworkshop=$_POST["dateworkshop"];
        $cat=$_POST["cat"];

    if ($_FILES["Workshop_img"]["name"] != '') {
    
        $exe = pathinfo($_FILES["Workshop_img"]["name"], PATHINFO_EXTENSION);
        $filename = time().random_int(1111, 9999) . "." .$exe;
        move_uploaded_file($_FILES["Workshop_img"]["tmp_name"], "./uploads/" .$filename);
        ...
        $result = mysqli_query($conn,"UPDATE tbl_workshop SET Title='$title',Description='$desc',Date='$dateworkshop',Category='$cat', Workshop_img='$filename' WHERE Workshop_ID='$Workshop_ID';") or die (mysqli_error($conn));
        //echo $result;die;
        if($result){
        echo "<script>
...
<html lang="en" class="light">
    ...
    <body class="py-5 md:py-0 bg-black/[0.15] dark:bg-transparent">
        ...
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">View Workshop</a></li>
                                ...
                                <div class="preview">
                                    <div class="mt-3">
                                       <form method="post" enctype="multipart/form-data">
                                        <label for="regular-form-2" class="form-label">Title</label>
                                        <input id="regular-form-2" type="text" class="form-control form-control-rounded" placeholder="Enter Title" name="title" value="<?php echo $title_value;?>"><br><br>
                                        <label for="regular-form-2" class="form-label">Description</label>
                                        <input id="regular-form-2" type="text" class="form-control form-control-rounded" placeholder="Enter Description" name="desc" value="<?php echo $desc_value;?>"><br><br>
                                        <label for="regular-form-2" class="form-label">Date</label>
                                        <input id="regular-form-2" type="date" class="form-control form-control-rounded" placeholder="Rounded" name="dateworkshop" value="<?php echo $date_value;?>"><br><br>
                                        <label for="regular-form-2" class="form-label">Category</label>
                                        <div class="flex flex-col sm:flex-row items-center" name="cat">
                                        <select class="form-control form-control-rounded" aria-label="Default select example" name="cat" selected=<?php echo $cat_value;?>>
                                                <option value="Technical" <?php echo($cat_value=="Technical")?'selected':''?>>Technical</option>
                                                <option value="Non-Technical" <?php echo($cat_value=="Non-Technical")?'selected':''?>>Non-Technical</option>
                                        </select>
                                        </div><br><br>
                                        <div class="mt-3">
                                            <label for="Workshop_img" class="form-label">Upload Image</label>
                                            <input type="file" class="form-control form-control-rounded" name="Workshop_img">
                                        </div>

                                        
                                        <div class="mt-3">
                                            <label class="form-label">Current Image</label><br>
                                            <img src="uploads/<?php echo $row['Workshop_img']; ?>" width="150" height="100" style="border-radius: 10px; border: 1px solid #ccc;">
                                        </div>
